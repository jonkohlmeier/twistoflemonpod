...
/**
 * Normalize and sort taxonomy entries (categories, tags, etc.) by display name
 * @param {Object|Array} taxonomy - Taxonomy map or array
 * @returns {Array<Object>} Sorted array with slug/name/posts
 */
function sortTaxonomyByName(taxonomy) {
  if (!taxonomy) return [];

  const list = Array.isArray(taxonomy)
    ? taxonomy.map(item => ({ ...item }))
    : Object.entries(taxonomy).map(([slug, data = {}]) => ({
        slug,
        ...data
      }));
...
function sortCategoriesByName(categories) {
  return sortTaxonomyByName(categories);
}

function sortTagsByName(tags) {
  return sortTaxonomyByName(tags);
}
